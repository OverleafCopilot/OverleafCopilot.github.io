...
function render_highlight_row(indexes, row_id, col_class) {
    fetch(indexes).then((response) => response.json()).then((highlights) => {
...
function render_agent_row(indexes, row_id, col_class) {
    fetch(indexes).then((response) => response.json()).then((featured_agents) => {
        featured_agents.forEach((data) => {
            var { name, desc, level, author, image, source, private, version, contact } = data;
            var card_body = $('<div class="card-body">');
            var card_title = $(`
            ...
            var card = $('<div class="card shadow-sm">');
            if (image) {
                card.append(`<img src="${image}" class="card-img-top img-fluid rounded-start" alt="${image}">`);
            }
            card.append(card_body);
            $(`#${row_id}`).append($(`<div class="col ${col_class}">`).append(card));
        })
